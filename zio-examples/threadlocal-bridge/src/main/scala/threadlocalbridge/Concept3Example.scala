...
/**
 * Title: ThreadLocalBridge with Inheritance and Cleanup Description: This
 * example demonstrates how ThreadLocalBridge handles value inheritance in
 * nested fibers and resource cleanup. Run: sbt "threadlocal-bridge/runMain
 * threadlocalbridge.Concept3Example"
 */
object Concept3Example extends ZIOAppDefault {
  ...
  val complexInheritanceExample: ZIO[Scope with ThreadLocalBridge, Nothing, Unit] = for {
    ...
    txRef <- ThreadLocalBridge.makeFiberRef("tx-parent-001")(value => transactionIdThreadLocal.set(value))
  ...
  val multipleInheritanceExample: ZIO[Scope with ThreadLocalBridge, Nothing, Unit] = for {
    ...
    txRef <- ThreadLocalBridge.makeFiberRef("tx-parallel-root")(value => transactionIdThreadLocal.set(value))
