...
/**
 * Title: Creating and Using ThreadLocalBridge Description: This example shows
 * how to use ThreadLocalBridge to safely manage ThreadLocal values across ZIO
 * fiber boundaries. Run: sbt "threadlocal-bridge/runMain
 * threadlocalbridge.Concept2Example"
 */
object Concept2Example extends ZIOAppDefault {
  ...
  val exampleWithBridge: ZIO[Scope with ThreadLocalBridge, Nothing, Unit] = for {
    ...
    userContextRef <- ThreadLocalBridge.makeFiberRef("user-alice")(value => userContextThreadLocal.set(value))
