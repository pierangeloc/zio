...
/**
 * Title: Composing Differs — zip, orElseEither, and transform Description:
 * Demonstrates how to compose Differs with <*> (zip) for product types, <+>
 * (orElseEither) for Either values, and transform for isomorphic types. Run:
 * sbt "differ-compositional-updates/runMain
 * differcompositionalupdates.ComposingDiffersExample"
 */
